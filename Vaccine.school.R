...
vaccine.schoolage<- rbind(  vaccine.town %>%
                              filter(Town !=" Boston") %>%
                              filter(Age_group == "0-4 Years" | Age_group == "5-11 Years" | 
                                Age_group == "12-15 Years" | Age_group == "16-19 Years" | Age_group == "0-19 Years" ) %>%
                            ...
                            left_join(
                              left_join(
                                vaccine.anydose.age %>%
                                  ...
                                  pivot_longer(cols = c(age0to4:age80plus.boost), names_to = "Age_group", values_to = "vaxnum") %>%
                                ...
                                ALLzips.age %>%
                                  ...
                                  mutate(
                                    age0to4 = case_when(
                                      variable == "B01001_003" ~ pop,  #0to4 male, 
                                      variable == "B01001_027" ~ pop),  #0to4 female, 
                                    age5to11 = case_when(
                                  ...
                                  group_by(zipcode) %>%
                                  summarise(age0to4 = round(sum(age0to4, na.rm = TRUE),0),
                                            age5to11 = round(sum(age5to11, na.rm = TRUE),0),
                                            age12to15 = round(sum(age12to15, na.rm = TRUE),0),
                                            age16to19 = round(sum(age16to19, na.rm = TRUE),0)) %>%
                                  pivot_longer(cols = c(age0to4:age16to19), names_to = "Age_group",values_to = "pop"), by=c("zipcode", "Age_group")),
...
vaccine.schoolage <- 
  left_join(
      rbind(vaccine.schoolage %>% filter(Town %!in% sharedtowns$Town.B),    # add in values for towns share zips and thus vaccine data
...
allma.0to4vax<-ggplot()+
  geom_polygon(data = vaccine.schoolage.geo %>% filter(Age_group == "0-4 Years") %>%
                 mutate(anydose.pct= case_when(anydose.pct >0.81 ~ 0.81, 
                                               anydose.pct <=0.81 ~ anydose.pct)), 
               aes(x = long, y = lat, group = group, fill=anydose.pct*100), 
  ...
  scale_fill_distiller(palette='YlOrRd', direction=1, na.value = "light grey",
                       limits=c(0,21),breaks = c(0, 20, 40, 60, 80),
                       name="Percent of 0 to 4\nyear-old residents") +
  labs(title="First dose vaccination among 0 to 4 year-olds, Massachusetts",
       subtitle = paste0("Data through ", max(vaccine.schoolage$reportdate, na.rm = TRUE)),
       caption="Source: MDPH. Towns with <30 vaccinations among 0-4 supressed for privacy reasons (colored grey)")
allma.0to4vax
ggsave("allma 0to4vax.pdf", width=16, height=9)
...
write_csv(vaccine.schoolage, "vaccine.schoolage.csv")
